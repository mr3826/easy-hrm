...
class OrganisationView extends GetView<ProfileGlobalController> {
final  String orgId;
  OrganisationView({super.key,required this.orgId});
  ...
  Widget build(BuildContext context) {
    _getSelectedIndex();
    return Obx(() {
      if (controller.isViewOrganizationLoading.isTrue) {
      ...
      return Column(
        children: [
          orgButtonSheetAppbar(
              orgLength: Get.find<ProfileGlobalController>()
                      .organizationInfo
          ...
          Expanded(
            ...
            child: ListView.builder(
              itemCount: Get.find<ProfileGlobalController>()
                  .organizationInfo
                  ?.getUserOrganizations
                  ?.data
                  ?.length,
                ...
                return InkWell(
                  onTap: () {
                    if (GetStorage().read(AppString.LAST_INPUT) != null) {
                      ...
                      Get.find<ProfileGlobalController>().switchOrganization(
                          orgId: Get.find<ProfileGlobalController>()
                                  .organizationInfo
                                  ?.getUserOrganizations
                                  ?.data?[index]
                                  .organization
  ...
  _organisationLogoLayout(int index) {
    return CustomNetworkImage(
      ...
      height: 22,
      errorText:
      getInitials(Get.find<ProfileGlobalController>()
              .organizationInfo
              ?.getUserOrganizations
              ?.data?[index]
              .organization
      ...
      orgId: Get.find<ProfileGlobalController>()
          ...
          ?.id,
      isPublic: true,
      imgUrlKey: Get.find<ProfileGlobalController>()
              .organizationInfo
              ?.getUserOrganizations
              ?.data?[index]
              .organization
  ...
  void _getSelectedIndex() {
    int? index = Get.find<ProfileGlobalController>()
        .organizationInfo
        ?.getUserOrganizations
        ?.data
        ?.indexWhere((element) =>
            element.organization?.id == orgId);
