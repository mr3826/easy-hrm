...
class BuildTimesheetList extends GetView<TimeSheetController> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      } else if (controller.timeSheetModel?.getUsersTimeSheet?.data?.isEmpty ??
          false) {
        return Center(
            child: Text(
      ...
      onTap: () async{
        ...
        Get.to(() => TimeLogSummary(
              ...
              logSummaryUserInfo: LogSummaryUserInfo(
                  ...
                  departmentName: data.organizationUser?.department?.name ?? "",
                          ...
                          Flexible(
                              child: _buildDetailRow(
                                  ...
                                  getConvertSecondsToHours(
                                      data.totalScheduledSeconds.toString()))),
                          const SizedBox(width: 12),
                          Flexible(
                              child: _buildDetailRow(
                                  'Logged:',
                                  getConvertSecondsToHours(
                                      data.loggedTotalSeconds.toString()))),
                        ],
                      ),
  ...
  String _getTime(Data data) {
    ...
    final endTime = data.timelineEndDate == null
        ? "Ongoing"
    ...
    children: [
      ...
      Flexible(
        child: Text(
          value,
          overflow:
              TextOverflow.ellipsis, // Prevent overflow if the text is too long
          style: AppStyle.normal_text.copyWith(
...
Widget _buildBalanceRow(Data data) {
  return Row(
    ...
    children: [
      ...
      const SizedBox(width: 4),
      Text(getConvertSecondsToHours(data.loggedTotalSeconds.toString()),
    ...
    children: [
      CircularNetworkImage(
        imageUrl: buildImgIxUrl(
            imagePath: data.organizationUser?.profile?.image ?? "",
            isPublic: true),
        radius: 18,
