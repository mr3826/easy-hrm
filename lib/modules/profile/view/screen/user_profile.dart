...
class ProfileScreen extends GetView<UserProfileController> {
        ...
        (sate) => Scaffold(
              ...
              body: ProfileScreen1(
                userInformation: UserInformation(
                    userName: "${controller.userDetails?.getOrganizationUserDetails?.profile?.firstName ?? ""} ${controller.userDetails?.getOrganizationUserDetails?.profile?.lastName ?? ""}",
                    departmentName: controller.userDetails?.getOrganizationUserDetails?.department?.name ??"",
                  employeeId: controller.userDetails?.getOrganizationUserDetails?.employeeId ??"",
                  employmentStatus: [controller.userDetails?.getOrganizationUserDetails?.status],

                  leaveBalance: controller.userLogHistory
                      ?.geTimelogAndLeaveAvailabilityForApp?.balanceLeave.toString() ??
                      "",

                  monthGoal: controller.userLogHistory
                      ?.geTimelogAndLeaveAvailabilityForApp?.totalSchedule ??
                      "",

                  loggedTime: controller.userLogHistory
  ...
  Widget _buildDepartmentLayout(BuildContext context) {
    List<DeptHistories>? departmentHistory = controller
        .employeeWorkHistory?.getOrganizationUserHistory?.deptHistories;
    if (departmentHistory != null && departmentHistory.isNotEmpty) {
      return departmentLayout(context);
    }
    return const SizedBox.shrink();
  ...
  Widget _buildDesignationHistoryLayout(BuildContext context) {
    List<DesignationHistories>? designationHistory = controller
        .employeeWorkHistory?.getOrganizationUserHistory?.designationHistories;
    if (designationHistory != null && designationHistory.isNotEmpty) {
      return employeeStatusLayout(context: context);
    }
    return const SizedBox.shrink();
