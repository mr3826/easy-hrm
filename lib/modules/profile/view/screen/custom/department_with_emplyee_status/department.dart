...
class DepartmentLayout extends StatelessWidget {
  ...
  Widget _buildDepartmentInfo() {
    return GestureDetector(
      onTap: () => userInformation.departmentInfo?.onAction(),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  Widget _buildParentInfo() {
    return Row(
      children: [
        Text(
          userInformation.departmentInfo?.parentDepartmentName ?? "",
          style: AppStyle.mid_large_text.copyWith(
            color: AppColor.secondaryColor,
            fontSize: Dimensions.fontSizeDefault - 1,
          ),
        ),
        if (userInformation.departmentInfo?.parentDepartmentName != null) _buildDivider(),
      ],
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  userInformation.departmentInfo?.workShiftName??"",
                  style: AppStyle.mid_large_text.copyWith(
  ...
  Widget _buildShiftTiming() {
    return Row(
      children: [
        Text(
          "${_formatTime(userInformation.departmentInfo?.workShiftStartTime??"")} - ${_formatTime(userInformation.departmentInfo?.workShiftEndTime??"")}",
          style: AppStyle.mid_large_text.copyWith(
            color: AppColor.hintColor,
            fontSize: Dimensions.fontSizeDefault + 1,
            overflow: TextOverflow.ellipsis,
        ...
        GestureDetector(
          onTap: () {},
          child: Text(
            _getTimeDifference(userInformation.departmentInfo?.workShiftStartTime??"",
                userInformation.departmentInfo?.workShiftEndTime??""),
            style: AppStyle.mid_large_text.copyWith(
              color: AppColor.normalTextColor.withOpacity(0.7),
              fontSize: Dimensions.fontSizeDefault - 1,
              overflow: TextOverflow.ellipsis,
  ...
  Widget _buildWorkingDaysSchedule(BuildContext context) {
    return SizedBox(
      ...
      child: ListView.builder(
        shrinkWrap: true,
        itemCount: userInformation.departmentInfo?.workingDays.length,
        scrollDirection: Axis.horizontal,
        physics: const BouncingScrollPhysics(),
        itemBuilder: (context, index) {
          final dayInfo = userInformation.departmentInfo?.workingDays[index];
          return Padding(
